...
(function () {
  ...
  function renderStatusRegion() {
    ...
    var html = '<div class="ow-sec-h">' + ic('activity') + esc(t('status')) + '</div>' +
  ...
  function renderConfigRegion() {
    var html = '<div class="ow-sec-h">' + ic('settings') + esc(t('config')) + '</div>' +
  ...
  function renderUsageRegion() {
    ...
    if (!cfgReady) {
      ...
      if (usage.priced && typeof s.cost === 'number') {
        stats.push([t('cost'), (usage.currency || '¥') + ' ' + fmtMoney(s.cost)])
      }
      ...
      if (rows.length) {
        ...
        if (restN > 0) {
          var o = { model: t('others').replace('{0}', restN), calls: 0, in: 0, out: 0, cached: 0, lat: 0, err: 0, cost: 0 }
        ...
        // Price table editor: what you pay per million tokens, per model seen in
        // the current range. Unpriced models stay in the stats but cost nothing.
        ...
        body += '<div class="ow-price">' +
          '<div class="ow-price-h"><span>' + esc(t('pricing')) + '</span>' +
    ...
    secUsage.innerHTML =
      '<div class="ow-sec-h">' + ic('chart-bar') + esc(t('usage')) + '</div>' +
      '<div class="ow-card ow-usage">' +
        '<div class="ow-tabs">' + tabs + '</div>' + body +
      '</div>'
    // Re-apply carried-over values onto the freshly built inputs.
    try {
      secUsage.querySelectorAll('input[data-pm]').forEach(function (el) {
      ...
      if (curEl && prior['@cur'] !== undefined && curEl.value !== prior['@cur']) curEl.value = prior['@cur']
  ...
  function renderLogRegion() {
    ...
    if (!lines.length) {
      secLog.innerHTML =
        '<div class="ow-sec-h">' + ic('file-text') + esc(t('log')) + '</div>' +
