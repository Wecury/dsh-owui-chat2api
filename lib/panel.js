...
export function createPanelInstance(opts) {
  ...
  // this module is imported (defer preserves order). Degrade to empty dicts if
  // it ever fails to load - t() then shows the key itself instead of crashing.
  ...
  function renderUsageRegion() {
    ...
    if (!cfgReady) {
      ...
      var stats = [
        [t('calls'), fmt(s.calls), STAT_COLORS.calls], [t('inTok'), fmt(s.in_tokens), STAT_COLORS.inTok], [t('outTok'), fmt(s.out_tokens), STAT_COLORS.outTok],
