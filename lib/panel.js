...
(function () {
  ...
  var I18N = {
    en: {
      ...
      effortScan: 'Auto-configure DSH reasoning levels',
      effortScanHint: 'Probes models via the proxy and writes reasoningEfforts into ~/.dsh/settings.yaml (backed up first). Restart DSH to apply.',
    ...
    zh: {
      ...
      effortScan: '自动配置 DSH 推理等级',
      effortScanHint: '通过代理探测模型,把 reasoningEfforts 写入 ~/.dsh/settings.yaml(自动备份)。重启 DSH 生效。',
  ...
  function renderDiagRegion() {
    ...
    var html = '<div class="ow-card ow-diag">' +
      ...
      (d.message ? '<span class="ow-muted ow-diag-msg">' + esc(trMsg(d.message)) + '</span>' : '') +
      '</div>'
