...
(function () {
  ...
  function showSaveResult(m, type) {
    ...
    if (!el) return
    if (m) {
      el.className = 'ow-save-result'
      el.innerHTML = noteHtml(trMsg(m), noteType(type))
      el.style.display = ''
    } else {
      el.innerHTML = ''
  ...
  function doSave() {
    withBusy(api('config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form || {}) }).then(function (r) {
      ...
      // inline under Save, right where the click is - same .ow-note look as the
      // sticky notice bar, with its severity carried by the icon+colour
      showSaveResult(bad ? (r.startResult.message || t('save')) : t('saved'), bad ? 'err' : 'ok')
      if (!bad) loadUsage()
  ...
  function doEffortScan(force) {
    ...
    function show(m, type) {
      if (!resEl) return
      if (m) {
        resEl.className = 'ow-effort-result'
        resEl.innerHTML = noteHtml(m, noteType(type))
        resEl.style.display = ''
      } else {
        resEl.innerHTML = ''
    ...
    // run that corrupts settings.yaml writes.
    withBusy(api(force ? 'effort-scan-force' : 'effort-scan', { method: 'POST' }).then(function (r) {
      if (!r || !r.ok) { show(trMsg((r && r.message) || (t('effortScan') + '?')), 'err'); return }
      var bits = []
      ...
      show(done, r.verify ? 'warn' : (r.changed ? 'ok' : 'info'))
    }).catch(function (e) { show(String((e && e.message) || e), 'err') }), 1800000)
  }
  function doStart() {
  ...
  function renderConfigRegion() {
    var html = '<div class="ow-sec-h">' + esc(t('config')) + '</div>' +
      '<div class="ow-card ow-config">' +
        ...
        '<div class="ow-scan-row"><div class="ow-scan-btns"><button class="ow-btn" data-effort-scan="1" type="button">' + esc(t('effortScan')) + '</button><button class="ow-btn ow-btn-ghost" data-effort-force="1" type="button" title="' + esc(t('effortRescanHint')) + '">' + esc(t('effortRescan')) + '</button></div><div class="ow-hint">' + esc(t('effortScanHint')) + '</div><div class="ow-effort-result" style="display:none"></div></div>' +
  ...
  function renderLogRegion() {
    ...
    secLog.innerHTML =
      ...
      '<details class="ow-log"' + (logOpen ? ' open' : '') + '>' +
        '<summary><span class="ow-log-sum-lbl">' + esc(t('log')) + ' · ' + lines.length + '</span>' +
  ...
  function cssText() {
    return (
      ...
      // Sticky notice bar: transient system messages live right under the header
      // so they are visible no matter how far the panel is scrolled. Solid panel
      // background so scrolled content never shows through behind the bar.
      '#dsh-owui-panel .ow-notice { position: sticky; top: 0; z-index: 5; padding: 10px 20px 0; margin-bottom: -4px;' +
        ' background: var(--dsw-specific-sidebar-fill, #202020); }' +
      // One shared notification look for every message (banner, save result,
      // effort result, diagnostics detail): icon chip in a coloured ring, tinted
      // background + border, four severities.
      '#dsh-owui-panel .ow-note { display: flex; gap: 8px; align-items: flex-start; font-size: 12px; line-height: 1.55;' +
        ' padding: 8px 10px; border-radius: 8px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,.28)); }' +
      '#dsh-owui-panel .ow-note-ic { flex: 0 0 auto; width: 16px; height: 16px; display: inline-flex; align-items: center; justify-content: center;' +
        ' border-radius: 50%; font-size: 10px; font-weight: 700; margin-top: 2px; }' +
      '#dsh-owui-panel .ow-note-tx { flex: 1; min-width: 0; word-break: break-word; }' +
      '#dsh-owui-panel .ow-note-ok { color: var(--dsw-alias-state-success-primary);' +
        ' background: color-mix(in srgb, var(--dsw-alias-state-success-primary) 9%, transparent);' +
        ' border-color: color-mix(in srgb, var(--dsw-alias-state-success-primary) 26%, transparent); }' +
      '#dsh-owui-panel .ow-note-ok .ow-note-ic { background: color-mix(in srgb, var(--dsw-alias-state-success-primary) 18%, transparent); color: var(--dsw-alias-state-success-primary); }' +
      '#dsh-owui-panel .ow-note-warn { color: var(--dsw-alias-state-warn-primary);' +
        ' background: color-mix(in srgb, var(--dsw-alias-state-warn-primary) 10%, transparent);' +
        ' border-color: color-mix(in srgb, var(--dsw-alias-state-warn-primary) 28%, transparent); }' +
      '#dsh-owui-panel .ow-note-warn .ow-note-ic { background: color-mix(in srgb, var(--dsw-alias-state-warn-primary) 20%, transparent); color: var(--dsw-alias-state-warn-primary); }' +
      '#dsh-owui-panel .ow-note-err { color: var(--dsw-alias-state-error-primary);' +
        ' background: color-mix(in srgb, var(--dsw-alias-state-error-primary) 9%, transparent);' +
        ' border-color: color-mix(in srgb, var(--dsw-alias-state-error-primary) 28%, transparent); }' +
      '#dsh-owui-panel .ow-note-err .ow-note-ic { background: color-mix(in srgb, var(--dsw-alias-state-error-primary) 18%, transparent); color: var(--dsw-alias-state-error-primary); }' +
      '#dsh-owui-panel .ow-note-info { color: var(--dsw-alias-label-primary);' +
        ' background: color-mix(in srgb, var(--dsw-alias-label-primary) 6%, transparent);' +
        ' border-color: var(--dsw-alias-border-l1, rgba(128,128,128,.28)); }' +
      '#dsh-owui-panel .ow-note-info .ow-note-ic { background: color-mix(in srgb, var(--dsw-alias-brand-primary) 16%, transparent); color: var(--dsw-alias-brand-primary); }' +
      ...
      '#dsh-owui-panel .ow-diag-note { margin-top: 8px; width: 100%; }' +
      ...
      '#dsh-owui-panel .ow-effort-result { margin-top: 8px; }' +
      '#dsh-owui-panel .ow-effort-result:empty { display: none; }' +
      '#dsh-owui-panel .ow-scan-btns { display: flex; gap: 8px; flex-wrap: wrap; }' +
      '#dsh-owui-panel .ow-btn-ghost { background: transparent; color: var(--dsw-alias-label-secondary, inherit); border: 1px solid var(--dsw-alias-border-default, rgba(128,128,128,.4)); }' +
      '#dsh-owui-panel .ow-save-result { margin-top: 10px; }' +
      ...
      // Log: per-line rows with a severity dot; the summary bar carries the
      // follow (stick to tail) and copy actions.
      '#dsh-owui-panel .ow-log summary { display: flex; align-items: center; justify-content: space-between; gap: 8px;' +
        ' font-size: 11px; color: var(--dsw-alias-label-secondary, #9ca3af); cursor: pointer;' +
      ...
      '#dsh-owui-panel .ow-log-sum-lbl { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }' +
      '#dsh-owui-panel .ow-log-acts { display: inline-flex; gap: 4px; flex: 0 0 auto; }' +
      '#dsh-owui-panel .ow-log-btn { font: 500 11px/1 ui-monospace, SFMono-Regular, Menlo, monospace; padding: 3px 7px;' +
        ' border-radius: 6px; cursor: pointer; background: transparent; color: var(--dsw-alias-label-secondary, #9ca3af);' +
        ' border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,.3)); }' +
      '#dsh-owui-panel .ow-log-btn:hover { background: var(--dsw-alias-bg-layer-2, rgba(128,128,128,.14)); color: var(--dsw-alias-label-primary, #fff); }' +
      '#dsh-owui-panel .ow-log-btn.on { background: color-mix(in srgb, var(--dsw-alias-brand-primary) 16%, transparent);' +
        ' color: var(--dsw-alias-brand-primary); border-color: color-mix(in srgb, var(--dsw-alias-brand-primary) 42%, transparent); }' +
      '#dsh-owui-panel .ow-log-body { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 10.5px;' +
        ' background: var(--dsw-alias-bg-layer-2, rgba(0,0,0,.28)); border-radius: 8px; padding: 8px 10px; margin-top: 6px;' +
        ' max-height: 190px; overflow: auto; }' +
      '#dsh-owui-panel .ow-log-line { display: flex; gap: 7px; align-items: baseline; white-space: pre-wrap; word-break: break-word;' +
        ' line-height: 1.55; padding: 1px 0; color: var(--dsw-alias-label-secondary, #9ca3af); }' +
      '#dsh-owui-panel .ow-log-dot { flex: 0 0 auto; width: 6px; height: 6px; border-radius: 50%; margin-top: 4px;' +
        ' background: var(--dsw-alias-border-l2, rgba(128,128,128,.5)); }' +
      '#dsh-owui-panel .ow-log-line-err .ow-log-dot { background: var(--dsw-alias-state-error-primary); }' +
      '#dsh-owui-panel .ow-log-line-warn .ow-log-dot { background: var(--dsw-alias-state-warn-primary); }' +
      '#dsh-owui-panel .ow-log-line-err .ow-log-txt { color: color-mix(in srgb, var(--dsw-alias-state-error-primary) 82%, var(--dsw-alias-label-primary, #e5e7eb)); }' +
      '#dsh-owui-panel .ow-log-line-warn .ow-log-txt { color: color-mix(in srgb, var(--dsw-alias-state-warn-primary) 74%, var(--dsw-alias-label-primary, #e5e7eb)); }' +
  ...
  function anchorTick() { if (document.hidden) return; anchorOverlay() }
  setInterval(anchorTick, 800) // keep re-anchoring while the app mounts/changes
