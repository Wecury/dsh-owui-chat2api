...
import { startEffortScan, scanState } from './effort-scan.js'
import { loadPrices, savePrices, costFor } from './prices.js'
...
function apply(ctx) {
  ...
  disposers.push(web.register({ kind: 'exact', path: ROUTE + '/api/status', handler: (req, res) => {
    sendJson(res, 200, {
      ...
      effortScan: { running: scanState.running, startedAt: scanState.startedAt, resultAt: scanState.resultAt, result: scanState.result },
  ...
  // Async response pattern (same as start/login): return at once, the scan
  // runs on the host, the panel converges via /api/status (effortScan field).
  disposers.push(web.register({ kind: 'exact', path: ROUTE + '/api/effort-scan', handler: (req, res) => {
    if (req.method !== 'POST') { res.writeHead(405); res.end(); return }
    sendJson(res, 200, startEffortScan(false))
  }}))

  // Force re-probe: ignores the effort cache (--effort-force) and re-scans every
  // model. Same startEffortScan pipeline, just force=true.
  disposers.push(web.register({ kind: 'exact', path: ROUTE + '/api/effort-scan-force', handler: (req, res) => {
    if (req.method !== 'POST') { res.writeHead(405); res.end(); return }
    sendJson(res, 200, startEffortScan(true))
  }}))
  ...
  // The per-model cost overlay is computed here, host-side, from the manual
  // price table (lib/prices.js) - python and the usage DB stay untouched.
  disposers.push(web.register({ kind: 'exact', path: ROUTE + '/api/usage', handler: async (req, res) => {
    ...
    try {
      ...
      const { currency } = loadPrices()
      let unpriced = []
      let total = 0
      let anyCost = false
      if (data && Array.isArray(data.per_model)) {
        for (const m of data.per_model) {
          const c = costFor(m.model, m.in_tokens, m.out_tokens, m.cached_tokens)
          if (c.priced) { m.cost = c.cost; total += c.cost; anyCost = true }
          else if (unpriced.indexOf(m.model) === -1) unpriced.push(m.model)
        }
      }
      if (data && data.summary) data.summary.cost = anyCost ? total : 0
      if (data) { data.currency = currency; data.priced = anyCost; data.unpriced = unpriced }
  ...
  // Manual price table (see lib/prices.js): GET returns the saved table,
  // POST merges and persists it in <DSH_HOME>/dsh-owui-chat2api-prices.json.
  disposers.push(web.register({ kind: 'exact', path: ROUTE + '/api/prices', handler: async (req, res) => {
    if (req.method === 'GET') {
      sendJson(res, 200, loadPrices())
      return
    }
    if (req.method !== 'POST') { res.writeHead(405); res.end(); return }
    try {
      const patch = JSON.parse((await readBody(req)) || '{}')
      sendJson(res, 200, { ok: true, ...(savePrices(patch || {})) })
    } catch (e) {
      sendJson(res, 400, { ok: false, message: String((e && e.message) || e) })
    }
  }}))
  ...
  disposers.push(web.tapIndex((html) => {
    ...
    // panel.js is an ES module exporting createPanelInstance(); this tiny
    // bootstrap module mounts the overlay instance once per page. Classic
    // defer scripts above run before module scripts in document order, so the
    // dictionaries are already in place when the bootstrap runs.
    const boot = '<script type="module">import{createPanelInstance as c}from"' + ROUTE + '/panel.js";' +
      'if(!window.__dshOwuiOverlay){window.__dshOwuiOverlay=true;' +
      'try{c({mode:"overlay"})}catch(e){console.error("[dsh-owui-chat2api] overlay panel failed",e)}}' +
      '</script>'
    const inject = link + i18n + boot
