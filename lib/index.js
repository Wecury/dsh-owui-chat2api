...
// chat2api (reusing its saved token + cached results), then add missing models
// plus reasoningEfforts to the matching entries in ~/.dsh/settings.yaml (back up
// first). Text-level merge contract lives in settings-patch.js; DSH restart is
// required to apply.
let effortScanBusy = false
async function runEffortScan(force = false) {
  // Host-side guard: a scan can run for minutes; refuse concurrent scans so two
  // clicks can never patch settings.yaml at the same time.
  if (effortScanBusy) {
    return { ok: false, message: 'a scan is already running - let it finish first' }
  }
  effortScanBusy = true
  try { return await runEffortScanInner(force) } finally { effortScanBusy = false }
}
...
// Static panel assets (panel.js script, panel.css stylesheet), read once and
// cached until restart - pack updates require a DSH restart anyway.
...
function apply(ctx) {
  ...
  disposers.push(web.tapIndex((html) => {
    ...
    // both tags are injected once (guarded on the css asset being referenced).
