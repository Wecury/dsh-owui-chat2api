...
import { addReasoningEfforts } from './settings-patch.js'
...
async function runEffortScan() {
  ...
  const supported = Object.keys(scan).filter((m) => scan[m])
  ...
  const res = addReasoningEfforts(source, { baseUrl: proxyUrl, modelIds: supported })
  if (!res.ok) return { ok: false, message: res.message || 'settings patch failed' }
